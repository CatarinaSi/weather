...
public class WeatherProvider {
    ...
    public Weather getCityWeather(String city) {
        ...
        final ResponseEntity<WeatherResponseApi> response = restTemplate.getForEntity(url + "/30", WeatherResponseApi.class);
        return Weather
                .builder()
                .temperature(response.getBody().getWeatherResponseCurrent().getTemperature())
                .time(response.getBody().getWeatherResponseCurrent().getTime())
